...
static void create_rel_table(microdb_table_t **out_table) {
...
MDB_TEST(selfcheck_clean_db_returns_ok) {
    microdb_selfcheck_result_t sc;
    uint32_t tsv = 0u;
    microdb_table_t *table;
    rel_row_t row;

    ASSERT_EQ(microdb_kv_put(&g_db, "k1", &(uint8_t){1u}, 1u), MICRODB_OK);
    ASSERT_EQ(microdb_kv_put(&g_db, "k2", &(uint8_t){2u}, 1u), MICRODB_OK);
    ASSERT_EQ(microdb_kv_put(&g_db, "k3", &(uint8_t){3u}, 1u), MICRODB_OK);

    ASSERT_EQ(microdb_ts_register(&g_db, "s", MICRODB_TS_U32, 0u), MICRODB_OK);
    for (tsv = 0u; tsv < 5u; ++tsv) {
        ASSERT_EQ(microdb_ts_insert(&g_db, "s", tsv + 1u, &tsv), MICRODB_OK);
    }

    create_rel_table(&table);
    row.id = 1u; row.v = 11u; ASSERT_EQ(microdb_rel_insert(&g_db, table, &row), MICRODB_OK);
    row.id = 2u; row.v = 12u; ASSERT_EQ(microdb_rel_insert(&g_db, table, &row), MICRODB_OK);
    row.id = 3u; row.v = 13u; ASSERT_EQ(microdb_rel_insert(&g_db, table, &row), MICRODB_OK);

    memset(&sc, 0, sizeof(sc));
    ASSERT_EQ(microdb_selfcheck(&g_db, &sc), MICRODB_OK);
...
MDB_TEST(selfcheck_kv_live_bytes_mismatch) {
    ...
    ASSERT_EQ(microdb_kv_put(&g_db, "k", &(uint8_t){1u}, 1u), MICRODB_OK);
    core->kv.live_value_bytes += 999u;
    ASSERT_EQ(microdb_selfcheck(&g_db, &sc), MICRODB_ERR_CORRUPT);
...
MDB_TEST(selfcheck_rel_bitmap_mismatch) {
    ...
    create_rel_table(&table);
    rel_row_t row;
    row.id = 1u; row.v = 1u; ASSERT_EQ(microdb_rel_insert(&g_db, table, &row), MICRODB_OK);
    row.id = 2u; row.v = 2u; ASSERT_EQ(microdb_rel_insert(&g_db, table, &row), MICRODB_OK);
