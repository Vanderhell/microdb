...
This guide shows how to author a real platform port for `loxdb`.
...
Use it as a template for mapping your platform driver to `lox_storage_t`.

## 1) What a port must provide

`loxdb` expects a filled `lox_storage_t` with:
...
`port/esp32/lox_port_esp32.c` is not just a platform utility.
...
3. Keep the same error mapping style (`LOX_OK` / `LOX_ERR_STORAGE` / `LOX_ERR_INVALID`).

## 3) Core storage contract and adapters

Core `loxdb` storage contract is strict:
...
- do not pass raw storage directly into `lox_init`
- use optional backend adapter flow (`lox_backend_open_prepare`) and aligned/managed adapter path
...
- if your erase/write path is asynchronous, `sync()` must complete pending operations before returning `LOX_OK`
- returning `LOX_OK` from `sync()` means durability contract for your selected backend policy is satisfied

## 5) Thread safety hooks (RTOS integration)

Storage hooks are only one part of a production port.
For multithreaded environments, wire lock callbacks in `lox_cfg_t`:
...
Internal lock behavior is gated by `LOX_THREAD_SAFE` (see `src/lox_lock.h`):

- `LOX_THREAD_SAFE=0`: lock macros are no-op
- `LOX_THREAD_SAFE=1`: callbacks are invoked around core operations
...
2. Fill `lox_storage_t` and run `lox_init` smoke test.
...
- Setting `write_size` to natural media granularity and calling `lox_init` directly.
