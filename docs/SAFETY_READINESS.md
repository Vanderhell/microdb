...
microdb is not a certified library. This document describes artifacts to support your project's certification process.
...
### CLAIM-MEM-01: No heap allocation after `microdb_init()` returns `MICRODB_OK`.

- Statement: Runtime operation paths do not allocate/free heap memory.
- Evidence: `src/microdb.c:microdb_init` performs the main heap allocation; operational APIs use preallocated arenas (`src/microdb_kv.c`, `src/microdb_rel.c`, `src/microdb_wal.c`).
...
- Evidence: `microdb_kv_get` validates `buf_len` and returns `MICRODB_ERR_OVERFLOW`; REL row helpers validate column sizes/types before copy (`src/microdb_rel.c`).
...
- Evidence: `src/microdb.c:microdb_validate_handle`, `src/microdb.c:microdb_deinit`.
...
- Evidence: `src/*.c` uses bounded locals and loops over compile-time/static bounds (`MICRODB_*` limits).
...
|---|---|---|---|---|
| Null handle passed to any API | `db == NULL` | Entry validation in public API | `MICRODB_ERR_INVALID` | Caller fixes API use |
| malloc fails at `microdb_init()` | Heap exhaustion | `malloc` result check | `MICRODB_ERR_NO_MEM` | Increase RAM budget / reduce profile |
| Storage HAL `read()` returns error | Backend I/O failure | Storage read wrapper checks | `MICRODB_ERR_STORAGE` | Retry/reopen/failover backend |
| WAL tail truncated by power loss | Partial final WAL entry | Replay stops at first invalid entry | `MICRODB_OK` | Durable snapshot remains valid |
| WAL header corrupt | Header CRC/magic invalid | WAL header validation | `MICRODB_OK` | WAL reset path, continue from snapshot |
| Both snapshot banks corrupt | CRC/header mismatch in both banks | Bank validation on boot | `MICRODB_ERR_CORRUPT` | Reinitialize DB image |
| KV full with REJECT policy | Entry/value store saturated | Insert path capacity checks | `MICRODB_ERR_FULL` | Delete keys / resize profile |
| KV full with OVERWRITE policy (LRU eviction) | Entry/value store saturated | Insert path + eviction branch | `MICRODB_OK` | Oldest entry evicted |
| REL table at max_rows | No free row slots | REL insert checks | `MICRODB_ERR_FULL` | Delete rows / increase table max_rows |
| TS ring full with DROP_OLDEST policy | Stream capacity reached | TS insert policy branch | `MICRODB_OK` | Oldest sample dropped |
| TS ring full with REJECT policy | Stream capacity reached | TS insert policy branch | `MICRODB_ERR_FULL` | Caller retries later / clears stream |
| Concurrent access without mutex (`MICRODB_THREAD_SAFE=0`) | Multi-thread access on no-op lock config | No runtime detector | `MICRODB_OK` (undefined behavior risk) | Integrator must provide lock hooks |
| TXN rollback (explicit) | `microdb_txn_rollback` called | Transaction state machine | `MICRODB_OK` | Staged mutations discarded |
| Schema mismatch on reopen (`schema_version` changed) | Table schema version drift | REL schema checks/migration path | `MICRODB_ERR_SCHEMA` | Apply migration callback or reset schema |
| Key TTL expired on get | Current time >= `expires_at` | KV expiry check on get | `MICRODB_ERR_EXPIRED` | Caller refreshes key |
| `write_size != 1` at init | Unsupported backend contract | Storage contract validation | `MICRODB_ERR_INVALID` | Use adapter or compliant backend |
...
- Rule 15.5 (single point of exit): common `goto unlock`/multi-return cleanup in `src/microdb_kv.c` and `src/microdb_rel.c`.
- Rule 11.5 (casts from `void *`): arena allocations cast to typed pointers in `src/microdb_kv.c` and `src/microdb_rel.c`.
- Rule 20.x (function-like macros): lock/logging macros (`MICRODB_LOCK`, `MICRODB_UNLOCK`, `MICRODB_LOG`) are widely used.
...
2. Provide real mutex hooks when concurrency is possible (`MICRODB_THREAD_SAFE=1` + lock callbacks).
...
7. Map all `microdb_err_t` codes into your firmware FMEA and recovery policy.
