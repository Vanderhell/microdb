...
embedded systems where missed deadlines are unacceptable. For `microdb`, WCET
is derived from:

- compile-time limits (`MICRODB_*` macros),
...
1. Include [microdb_wcet.h](/C:/Users/vande/Desktop/github/microdb/include/microdb_wcet.h).
...
- Formula:
  - `T_wcet = T_hash + MICRODB_WCET_KV_PROBE_MAX * T_strncmp(KEY_MAX) + T_memcpy(val_len)`

### `microdb_kv_set()` (no storage)
...
- WAL writes: none
- Formula:
  - `T_wcet = T_probe_wc + T_memmove(MICRODB_WCET_KV_MEMMOVE_MAX) + T_memcpy(val_len)`

### `microdb_kv_set()` (WAL sync always)

- Best case: append WAL entry + sync
- Worst case: same as above + worst storage write/sync + optional compact
- Formula:
  - `T_wcet = T_probe_wc + T_memmove(MICRODB_WCET_KV_MEMMOVE_MAX) + T_flash_write(MICRODB_WCET_WAL_KV_SET_MAX) + T_flash_sync + T_compact_wc`

### `microdb_kv_set()` (WAL flush-only)

- Best case: no immediate sync
- Worst case: deferred compact/flush still possible
- Formula:
  - `T_wcet = T_probe_wc + T_memmove(MICRODB_WCET_KV_MEMMOVE_MAX) + T_flash_write(MICRODB_WCET_WAL_KV_SET_MAX) + T_compact_wc`
...
- Formula:
  - `T_wcet = T_hash + MICRODB_WCET_KV_PROBE_MAX * T_strncmp(KEY_MAX) + T_flash_write(WAL_DEL_MAX)`
...
- WAL writes:
  - scalar: `MICRODB_WCET_WAL_TS_F32_INSERT_MAX`
  - raw: `MICRODB_WCET_WAL_TS_RAW_INSERT_MAX`
- Formula:
  - `T_wcet = T_ring_update + T_policy_overflow_wc + T_flash_write(WAL_TS_MAX) + T_flash_sync(mode)`

### `microdb_ts_query()` / `microdb_ts_query_buf()`
...
- Formula:
  - `T_wcet = T_bitmap_scan + T_shift(MICRODB_WCET_REL_INDEX_SHIFTS_MAX(max_rows)) + T_flash_write(MICRODB_WCET_WAL_REL_INSERT_MAX(row_size))`
...
2. Measure flash write/sync latency with same partition/backend used by `microdb`.
3. Substitute values into formulas and add safety margin.
4. Validate with long-run workload replay and `microdb_selfcheck()`.
