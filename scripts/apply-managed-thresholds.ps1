...
foreach ($m in $map) {
    ...
    $oldSmoke = [string]$preset.cacheVariables.LOX_MANAGED_STRESS_SMOKE_MAX_MS
    $oldLong = [string]$preset.cacheVariables.LOX_MANAGED_STRESS_LONG_MAX_MS
    $newSmoke = [string]$smokeRec
    $newLong = [string]$longRec

    if ($oldSmoke -ne $newSmoke) {
        $preset.cacheVariables.LOX_MANAGED_STRESS_SMOKE_MAX_MS = $newSmoke
    }
    if ($oldLong -ne $newLong) {
        $preset.cacheVariables.LOX_MANAGED_STRESS_LONG_MAX_MS = $newLong
    }
